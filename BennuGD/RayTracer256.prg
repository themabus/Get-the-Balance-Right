//320x240	15132, 15060, 15103, 15171, 15113
    ...
    switch (stage)
        case 0:
            ...
            while (pix<out_size)
                ...
                if (get_timer() - t > time_step)
                    delete_text(0);
                    write(0, 0,10, ALIGN_TOP_LEFT, pix+"/"+out_size);
                    return;
                end
            end
            pix = 0;
        ...
        case 1://unneccesary loops left in deliberately
            while (pix<out_size)
                ...
                if (get_timer() - t > time_step)
                    delete_text(0);
                    write(0, 0,10, ALIGN_TOP_LEFT, out_size+"/"+out_size);
                    return; 
                end;
            end
            pix = 0;
        ...
        case 2:
            ...
            write(0, 0,10, ALIGN_TOP_LEFT, time_end);
        end
        default:
            put_screen(0,map);
        end
...
begin
    ...
    elseif( (key(_R_ALT) || key(_L_ALT)) && key(_ENTER) )
        if (kbd_ready)
            fs = !fs;
            kbd_ready = false;
            if (fs) set_mode(width,height,32,MODE_FULLSCREEN);
            else set_mode(width,height,32,MODE_WINDOW);
            end
        end
    else
        kbd_ready = true;
