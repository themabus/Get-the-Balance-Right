//320x240	  959,   944,   950,   942,   949	<- dev ide with debug stuff
//320x240	  761,   664,   659,   690,   663	<- exported to exe

using Godot;
using System;

class main : Node2D {
	static int width = (int)ProjectSettings.GetSetting("display/window/size/width");
	static int height = (int)ProjectSettings.GetSetting("display/window/size/height");
	...
	public override void _Ready() {
		...
		label2.RectPosition = new Vector2(0,12);
		AddChild(label1);
		AddChild(label2);
		// <--- labels
	}
		...
		switch(stage) {
			case 0:
				...
				while (pix < out_size) {
					...
					pix++;
					if (OS.GetTicksMsec() - t > time_step) {
			...
			case 1: // unneccesary loops left in deliberately
				...
				Update();
				stage++;
				time_end = OS.GetTicksMsec() - time_start;
				GD.Print(time_end);
	...
	public override void _Input(InputEvent @event) {
		if (@event is InputEventKey keyEvent && keyEvent.Pressed && !keyEvent.IsEcho()) {
			if(Input.IsKeyPressed((int) KeyList.Enter) && Input.IsKeyPressed((int) KeyList.Alt)) {
