#320x240	 6135,  6393,  6292,  6182,  6173	<- dev ide with debug stuff
#320x240	 4702,  4698,  4753,  4769,  4670	<- exported to exe
...
func _ready():
	...
	label2.position = Vector2(0,10)
	label1.add_theme_font_size_override("font_size", 11)
	...
	match stage:
		0:
			...
			while pix < out_size:
				...
				pix+=1
				if Time.get_ticks_msec() - t > time_step:
		...
		1: # unneccesary loops left in deliberately
			...
			queue_redraw()
			stage+=1
			time_end = Time.get_ticks_msec() - time_start
			print(time_end)
